...
class API_Performance extends Simulation{
  ...
  val update_the_data = scenario(" Send the request to and check the status").exec(karateFeature("src/test/java/examples/users/users.feature"))
  val delete_the_user = scenario("Delete the user from the list").exec(karateFeature("src/test/java/examples/users/users.feature"))
  ...
  setUp {
    ...
    update_the_data.inject(rampUsers(5)during(10 second))
    delete_the_user.inject(rampUsers(10).during(10 second))
  }
}
